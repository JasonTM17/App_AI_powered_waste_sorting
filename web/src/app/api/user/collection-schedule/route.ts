...
import { cloudSchedules } from "@/lib/server/cloud-operations";
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await cloudSchedules(identity));
  } catch (error) {
    if (error instanceof CloudAuthConfigError) {
      return NextResponse.json({ detail: "Cloud database is not configured" }, { status: 503 });
    }
    return NextResponse.json({ detail: "Cloud user schedule failed" }, { status: 500 });
  }
}
