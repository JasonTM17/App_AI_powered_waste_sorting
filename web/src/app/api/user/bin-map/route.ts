...
import { cloudBinMap } from "@/lib/server/cloud-operations";
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await cloudBinMap(identity, false));
  } catch (error) {
    if (error instanceof CloudAuthConfigError) {
      return NextResponse.json({ detail: "Cloud database is not configured" }, { status: 503 });
    }
    return NextResponse.json({ detail: "Cloud user bin map failed" }, { status: 500 });
  }
}
