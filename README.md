...
- Camera-driven dispatch is guarded by defaults in `dispatch_guard`: enabling automatic sorting arms the first valid object immediately after `3` stable frames; one active dump remains locked until ACK/NACK/timeout plus `0.35s` settle; after ACK/HOME the camera must observe an empty tray for `2` seconds and at least `10` frames before accepting the next object; and ROI must be valid and enabled. Manual Test Huu co/Vo co/Tai che buttons remain direct hardware tests.
