...
- Camera-driven dispatch is guarded by defaults in `dispatch_guard`: enabling automatic sorting arms the first valid object immediately after `3` stable frames; one active dump remains locked until ACK/NACK/timeout plus `1s` settle; later objects require the tray to be empty for `2s` and `10` frames before re-arm; and ROI must be valid and enabled. Manual Test Huu co/Vo co/Tai che buttons remain direct hardware tests.
