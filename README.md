...
- Camera-driven dispatch is guarded by defaults in `dispatch_guard`: enabling automatic sorting arms the first valid object after `2` stable frames; one active dump remains locked until ACK/NACK/timeout plus `0.8s` settle; after ACK/HOME the camera must observe an empty tray for `1.2` seconds and at least `6` frames before accepting the next object, so scale vibration cannot create an immediate second dump; and ROI must be valid and enabled. Manual Test Huu co/Vo co/Tai che buttons remain direct hardware tests.
